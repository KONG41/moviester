        ...
        {  data && 
            data.map((item,index)=>(
                <div key={item.title} className="border-b-2 last:border-0 border-[#e6e4ec42]"> 
                    ...
                    <div className="flex flex-wrap md:justify-center justify-evenly ">
                        {
                          item.movies.map((movie,index_movie)=>(
                            <Link to={`${movie.link}`} key={`${index_movie}_${movie.title}`}>
                                <div className='md:w-[210px] w-[150px] m-2 mb-6'>
                                    <div className='md:w-[210px] w-[auto] md:h-[270px] h-[200px] rounded-lg bg-slate-400 hover:shadow-25xl relative group' style={{ backgroundImage: `url(${movie.cover})` }}>
                                    ...
                                    <div className='pt-2'>
                                        <div className="flex flex-row justify-between py-1 md:text-sm text-xs text-white">
                                            <span className="">{movie.year}</span>
                                            <span className="border border-black bg-[#ffb400] rounded-full px-1">{movie.type}</span>
                                            <span>{movie.duration}</span>
                                        </div>
                                        <h1 className="text-white font-semibold md:text-base text-xs">{movie.title}</h1>
                                    </div>
                                </div>
                            </Link>
                          ))
